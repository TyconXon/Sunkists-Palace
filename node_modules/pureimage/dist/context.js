...
export class Context {
    ...
    createConicGradient(angle, x0, y0) {
    ...
    fillRect(x, y, w, h) {
        if (this._transform.isIdentity()) {
            ...
            if (typeof this._fillColor == "number") {
                const new_rgba = getBytesBigEndian(this._fillColor);
                if (new_rgba[3] === 255) {
                    for (let i = x; i < x + w; i++) {
                        for (let j = y; j < y + h; j++) {
                            if (this.pixelInsideClip(i, j) &&
                                this._bitmap._isValidCoords(i, j)) {
                                this._bitmap.setPixelRGBA(i, j, this._fillColor);
                            }
                        }
                    }
                    return;
    ...
    composite(old_pixel, new_pixel) {
        ...
        // convert back to int
        return fromBytesBigEndian(Cf[0], Cf[1], Cf[2], // R, G, B,
    ...
    drawImage(bitmap, sx, sy, sw, sh, dx, dy, dw, dh) {
        ...
        let bitmap_bounds = new Bounds(0, 0, this._bitmap.width, this._bitmap.height);
        ...
        for (let i = dst_bounds.x1; i < dst_bounds.x2; i++) {
            for (let j = dst_bounds.y1; j < dst_bounds.y2; j++) {
                ...
                src_pt = new Point(remap(src_pt.x, dx, dx + dw, sx, sx + sw), remap(src_pt.y, dy, dy + dh, sy, sy + sh));
    ...
    roundRect(x, y, width, height, radii) {
        let corners = { bl: 0, br: 0, tr: 0, tl: 0 };
        ...
        this.quadraticCurveTo(x + width, y + height, x + width - corners.br, y + height);
    ...
    drawLine(line) {
        if (line.is_invalid())
            return console.error("cannot draw line", line);
